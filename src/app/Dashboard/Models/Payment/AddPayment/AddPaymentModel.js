...
const AddPaymentModel = ({ isOpen, onClose, fetchData }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await axios.post(`${API_URL_Payment}`, formData);
  ...
  return (
    <div className="fixed inset-0 flex items-center justify-center bg-gray-800 bg-opacity-50 z-50">
      <div className="bg-white p-6 rounded-lg shadow-lg w-full max-w-xl">
        <h2 className="text-3xl font-semibold text-center mb-8">Add Payment</h2>
        {error && <p className="text-red-600">{error}</p>}
        {success && (
          <p className="text-green-600">Payment added successfully!</p>
        )}
