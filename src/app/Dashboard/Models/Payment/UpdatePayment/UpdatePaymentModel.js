...
const UpdatePaymentModel = ({ isOpen, onClose, fetchData, paymentid }) => {
  ...
  useEffect(() => {
    ...
    const fetchManufacturerData = async () => {
      ...
      if (paymentid) {
        try {
          const response = await axios.get(`${API_URL_Payment}/${paymentid}`);
          ...
          if (data) {
            ...
            toast.warn("Failed to fetch Payment data");
          }
        } catch (err) {
          setError(
  ...
  return (
    <div className="fixed inset-0 flex items-center justify-center bg-gray-800 bg-opacity-50 z-50">
      <div className="bg-white p-6 rounded-lg shadow-lg w-full max-w-xl">
        ...
        {success && (
          <p className="text-green-600">Payment updated successfully!</p>
        )}
