...
export const PUT = async (request) => {
  try {
    ...
    const latestRecord = await VehicleMOT.findOne({
      motPending_Done: "1",
    ...
    if (!latestRecord) {
      return NextResponse.json(
        { error: "No record found with motPending_Done = 1", success: false },
        { status: 404 }
      );
    }

    // Update the found record to set motPending_Done to "0"
    latestRecord.motPending_Done = "0";
    ...
    console.error("Error updating motPending_Done field:", error);
    return NextResponse.json(
